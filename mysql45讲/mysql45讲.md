...
- [[#Mysql b+树|Mysql b+树]]
- [[#索引类型|索引类型]]
- [[#索引类型#索引深入浅出（下）——大纲和要点总结|索引深入浅出（下）——大纲和要点总结]]
...
- [[#索引类型#1. **直接执行 `delete from T limit 10000;`**|1. **直接执行 `delete from T limit 10000;`**]]
- [[#索引类型#2. **循环执行 20 次 `delete from T limit 500;`**|2. **循环执行 20 次 `delete from T limit 500;`**]]
- [[#索引类型#3. **在 20 个连接中同时执行 `delete from T limit 500;`**|3. **在 20 个连接中同时执行 `delete from T limit 500;`**]]
- [[#索引类型#答案：|答案：]]
- [[#1. 插入数据优化|1. 插入数据优化]]
...
- [[#总结与提升|总结与提升]]
- [[#总结与提升#为什么 MySQL InnoDB 选择 B+ 树作为索引的数据结构|为什么 MySQL InnoDB 选择 B+ 树作为索引的数据结构]]
- [[#总结与提升#一句话总结MySQL InnoDB 选择 B+ 树作为索引的数据结构？|一句话总结MySQL InnoDB 选择 B+ 树作为索引的数据结构？]]
- [[#总结与提升#什么时候适用索引|什么时候适用索引]]
- [[#总结与提升#什么时候不需要创建索引|什么时候不需要创建索引]]
- [[#总结与提升#什么时候索引会失效|什么时候索引会失效]]
- [[#总结与提升#优化索引的方法|优化索引的方法]]
- [[#总结与提升#一句话总结优化索引的方法|一句话总结优化索引的方法]]
	- [[#一句话总结优化索引的方法#1 “索引列设置为 `NOT NULL`，能够简化索引的存储结构，减少索引的存储空间占用”****|1 “索引列设置为 `NOT NULL`，能够简化索引的存储结构，减少索引的存储空间占用”****]]
- [[#InnoDB 是如何存储数据的？|InnoDB 是如何存储数据的？]]
- [[#InnoDB 是如何存储数据的？#InnoDB 数据存储与页目录|InnoDB 数据存储与页目录]]
- [[#InnoDB 是如何存储数据的？#B+ 树索引结构|B+ 树索引结构]]
- [[#InnoDB 是如何存储数据的？#聚簇索引与二级索引|聚簇索引与二级索引]]
- [[#InnoDB 是如何存储数据的？#一句话总结innodb是如何储存数据的？|一句话总结innodb是如何储存数据的？]]
- [[#索引失效条件|索引失效条件]]
- [[#一句话总结MySQL 使用 like “%x“，索引不一定失效的场景是什么|一句话总结MySQL 使用 like “%x“，索引不一定失效的场景是什么]]
- [[#一句话总结MySQL 使用 like “%x“，索引不一定失效的场景是什么#1. **基本结论**|1. **基本结论**]]
- [[#一句话总结MySQL 使用 like “%x“，索引不一定失效的场景是什么#2. **执行过程分析**|2. **执行过程分析**]]
- [[#一句话总结MySQL 使用 like “%x“，索引不一定失效的场景是什么#3. **其他存储引擎的差异**|3. **其他存储引擎的差异**]]
- [[#一句话总结MySQL 使用 like “%x“，索引不一定失效的场景是什么#4. **大表优化建议**|4. **大表优化建议**]]
- [[#事务的特性|事务的特性]]
- [[#事务的特性#1. 原子性（Atomicity）|1. 原子性（Atomicity）]]
- [[#事务的特性#2. 一致性（Consistency）|2. 一致性（Consistency）]]
- [[#事务的特性#3. 隔离性（Isolation）|3. 隔离性（Isolation）]]
- [[#事务的特性#4. 持久性（Durability）|4. 持久性（Durability）]]
- [[#InnoDB 引擎保证事务特性的技术|InnoDB 引擎保证事务特性的技术]]
- [[#并发事务引发的问题|并发事务引发的问题]]
- [[#并发事务引发的问题#1. 脏读（Dirty Read）|1. 脏读（Dirty Read）]]
- [[#并发事务引发的问题#2. 不可重复读（Non-repeatable Read）|2. 不可重复读（Non-repeatable Read）]]
- [[#并发事务引发的问题#3. 幻读（Phantom Read）|3. 幻读（Phantom Read）]]
- [[#事务的隔离级别|事务的隔离级别]]
- [[#事务的隔离级别#1. 读未提交（Read Uncommitted）|1. 读未提交（Read Uncommitted）]]
- [[#事务的隔离级别#2. 读已提交（Read Committed）|2. 读已提交（Read Committed）]]
- [[#事务的隔离级别#3. 可重复读（Repeatable Read）|3. 可重复读（Repeatable Read）]]
- [[#事务的隔离级别#4. 串行化（Serializable）|4. 串行化（Serializable）]]
- [[#总结|总结]]
- [[#事务的隔离级别有哪些？|事务的隔离级别有哪些？]]
- [[#1. 事务概述|1. 事务概述]]
- [[#2. 隔离性与问题|2. 隔离性与问题]]
- [[#2. 隔离性与问题#2.1 隔离级别|2.1 隔离级别]]
- [[#2. 隔离性与问题#2.2 解决方案|2.2 解决方案]]
- [[#2. 隔离性与问题#3. InnoDB的隔离性实现|3. InnoDB的隔离性实现]]
	- [[#3. InnoDB的隔离性实现#3.1 快照读（普通 SELECT 语句）|3.1 快照读（普通 SELECT 语句）]]
	- [[#3. InnoDB的隔离性实现#3.2 当前读（SELECT ... FOR UPDATE 等语句）|3.2 当前读（SELECT ... FOR UPDATE 等语句）]]
- [[#4. 隔离级别的实现机制|4. 隔离级别的实现机制]]
- [[#5. 结论|5. 结论]]
- [[#一句话总结undolog链条与mvcc:|一句话总结undolog链条与mvcc:]]
- [[#概述|概述]]
- [[#幻读定义|幻读定义]]
- [[#幻读解决方案|幻读解决方案]]
- [[#幻读解决方案#快照读（普通select语句）|快照读（普通select语句）]]
- [[#幻读解决方案#当前读（select ... for update等语句）|当前读（select ... for update等语句）]]
- [[#幻读未完全解决|幻读未完全解决]]
- [[#幻读场景分析|幻读场景分析]]
- [[#幻读场景分析#第一个场景|第一个场景]]
- [[#幻读场景分析#第二个场景|第二个场景]]
...
- [[#总结#1. 全局锁|1. 全局锁]]
- [[#总结#2. 表级锁|2. 表级锁]]
- [[#总结#3. 行级锁（InnoDB 支持）|3. 行级锁（InnoDB 支持）]]
- [[#总结#总结|总结]]
