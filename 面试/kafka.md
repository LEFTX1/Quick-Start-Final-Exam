...
### 你提到了分区和消费者组。那Kafka具体是怎么保证消息顺序性的呢？在什么场景下，这种严格的顺序性是必须的？
...
### Kafka它的持久化机制是怎样的？默认的存储方式是什么样的？或许你也可以简单对比一下它和Redis在持久化方面的区别。
...
### 这块就串起来了。那你能不能再展开讲讲零拷贝（Zero-Copy）技术，以及Kafka是怎么结合页缓存来利用它的？
...
###  "那么当消费者（Consumer）读取这些消息时，它们需要追踪自己在每个Partition里读到了哪里。Kafka是如何管理这个消费进度的？‘Offset’在这里扮演了什么样的角色？"
...
###  "理解了。那我们再深入一点，Kafka 的副本机制，也就是 Replication，它是怎么实现的呢？它又是如何保障数据可靠性的？"
...
###   很好。我们聊聊 Kafka 的事务。它声称能实现 Exactly-Once Semantics (EOS)，能简单说说它是怎么做到的吗？特别是那个 `transactional.id` 有什么用？
...
###   了解。那 Kafka 的多租户支持是如何实现的呢？比如我想让不同的团队或者业务线共享一个 Kafka 集群，同时又希望能控制他们各自的资源使用，这要怎么做？
